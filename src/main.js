...
import { appendLightningLog, updateLightningLog, getLightningLog, normalizeLightningStatus, isLightningEntryPending } from './lightning/log.js'
import { refreshCurrencyLabels, formatFiatFromSats } from './currency.js'
...
window._generateLightningInvoice = async ({ amount, description }) => {
  ...
  try {
    appendLightningLog({
      direction: 'receive',
      amountSats: Number(amount || result?.amount || 0),
      memo: description || '',
      status: 'Pending',
      paymentHash: result?.paymentHash || result?.pendingSwap?.paymentHash || null,
      invoice: result?.invoice || null,
      kind: 'reverse',
    })
  } catch (logErr) {
    console.warn('[ArkON] Failed to append Lightning receive log:', logErr)
  }
  return result
}
...
window._lightningLog = {
  list: () => getLightningLog(),
  add: (entry) => appendLightningLog(entry),
  update: (match, patch) => updateLightningLog(match, patch),
}
window._refreshCurrencyLabels = () => refreshCurrencyLabels()
window._formatFiatFromSats = (sats, currency, options) => formatFiatFromSats(sats, currency, options)
...
function getUnlockElements() {
  return {
    gate: document.getElementById('wallet-unlock-gate'),
    input: document.getElementById('wallet-unlock-password'),
    button: document.getElementById('wallet-unlock-submit'),
    error: document.getElementById('wallet-unlock-error'),
  }
}

function showUnlockGate(message = '') {
  const { gate, input, error } = getUnlockElements()
...
async function submitUnlockGate() {
  const { input, button } = getUnlockElements()
  const password = String(input?.value || '')
  if (!password) {
    showUnlockGate('Enter your wallet password')
    return
  }
  ...
  try {
    ...
    console.warn('[ArkON] Unlock failed:', err)
    showUnlockGate('Incorrect password')
  } finally {
    if (button) button.disabled = false
  }
}

function wireUnlockGate() {
  const { input, button } = getUnlockElements()
  if (!input || !button || button.dataset.wired === 'true') return
  button.dataset.wired = 'true'
  button.addEventListener('click', () => { submitUnlockGate() })
  input.addEventListener('keydown', (event) => {
    if (event.key === 'Enter') submitUnlockGate()
  })
}
...
let _lightningEventsWired = false
async function wireLightningMonitoring() {
  if (_lightningEventsWired) return
  _lightningEventsWired = true
  try {
    const swaps = await getLightningSwaps()
    const manager = swaps?.getSwapManager?.()
    if (!manager) return

    if (typeof manager.onSwapCompleted === 'function') {
      manager.onSwapCompleted((swap) => {
        try {
          const finalDirection = swap?.direction || (swap?.invoice ? 'send' : 'receive')
          updateLightningLog({
            paymentHash: swap?.paymentHash || swap?.claimDetails?.paymentHash || null,
            id: swap?.id || null,
            invoice: swap?.invoice || null,
          }, {
            direction: finalDirection,
            status: normalizeLightningStatus(swap?.status || swap?.state || 'Completed', finalDirection),
            txid: swap?.txid || swap?.claimTxid || swap?.claimDetails?.txid || null,
          })
        } catch (err) {
          console.warn('[ArkON] Lightning completion log update failed:', err)
        }
      })
    }

    if (typeof manager.onSwapFailed === 'function') {
      manager.onSwapFailed((swap) => {
        try {
          const failedDirection = swap?.direction || (swap?.invoice ? 'send' : 'receive')
          updateLightningLog({
            paymentHash: swap?.paymentHash || swap?.claimDetails?.paymentHash || null,
            id: swap?.id || null,
            invoice: swap?.invoice || null,
            ...
            status: normalizeLightningStatus(swap?.failureReason || swap?.status || 'Failed', failedDirection),
          })
        } catch (err) {
          console.warn('[ArkON] Lightning failure log update failed:', err)
        }
      })
    }
  } catch (err) {
    console.warn('[ArkON] Lightning monitoring unavailable:', err)
  }
}
...
async function ensureUnlockedIfNeeded() {
  ...
  if (_passwordPromptedThisSession) {
    hideUnlockGate()
    return true
  }
  showUnlockGate()
...
async function boot() {
  ...
  try {
    ...
    try { refreshCurrencyLabels() } catch {}
...
function wireConfirmSend() {
  window.confirmSend = async function () {
    ...
    // Lightning invoices already encode the destination, and usually the amount too.
    // Only require a manual amount for non-lightning destinations.
    if (!address) {
      if (typeof showToast === 'function') showToast('Missing address or invoice')
      return
    }

    if (routedType !== 'lightning' && (!amount || isNaN(amount) || amount <= 0)) {
    ...
    try {
      ...
      if (routedType === 'lightning') {
        ...
        try {
          decodedInvoice = window._decodeLightningInvoice ? window._decodeLightningInvoice(trimmed) : null
        } catch (decodeErr) {
          console.warn('[ArkON] Lightning invoice decode failed:', decodeErr)
        }
        ...
        if (!decodedInvoice.amountSats && (!amount || isNaN(amount) || amount <= 0)) {
          throw new Error('Amount required for zero-amount Lightning invoice')
        }

        if (btnLabel) btnLabel.textContent = 'Paying Lightning…'

        const paymentResult = await payLightningInvoice({ invoice: trimmed })

        try {
          appendLightningLog({
            direction: 'send',
            amountSats: Number(paymentResult?.amount ?? decodedInvoice?.amountSats ?? amount ?? 0),
            memo: decodedInvoice?.description || '',
            status: normalizeLightningStatus(paymentResult?.status || paymentResult?.state || 'Pending', 'send'),
            paymentHash: paymentResult?.paymentHash || decodedInvoice?.paymentHash || null,
            txid: paymentResult?.txid || null,
            invoice: trimmed,
            kind: 'submarine',
          })
        } catch (logErr) {
          console.warn('[ArkON] Failed to append Lightning send log:', logErr)
        }

        if (typeof closeSheet === 'function') closeSheet('sendconfirm')
        if (typeof showToast === 'function') {
          const sentAmt = Number(paymentResult?.amount ?? decodedInvoice?.amountSats ?? amount ?? 0)
          showToast(`Lightning payment sent${sentAmt > 0 ? ` • ${sentAmt.toLocaleString()} sats` : ''} ✓`)
        }
        ...
        try { if (typeof refreshTransactions === 'function') await refreshTransactions() } catch {}
