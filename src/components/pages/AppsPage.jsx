/**
 * AppsPage — purely structural.
 * App pill filtering controlled by ui.js setAppsFilter() via DOM.
 * React does NOT control filter state or section visibility.
 */
    ...
    <div style={{ padding: '0 20px 28px' }}>
      ...
      <div className="apps-pills" role="tablist" aria-label="Filter apps">
        {Object.entries(PILL_LABELS).map(([k, v], i) => (
          <button key={k} className={`app-pill${i === 0 ? ' active' : ''}`} data-app-filter={k} onClick={(e) => typeof setAppsFilter === 'function' && setAppsFilter(k, e.currentTarget)}>{v}</button>
        ))}
      </div>
