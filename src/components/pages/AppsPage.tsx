const PILL_LABELS: Record<string, string> = { all: 'All', finance: 'Finance', collectibles: 'Collectibles', games: 'Games' }
  ...
  return (
    <div style={{ padding: '0 20px 28px' }}>
      ...
      <div className="apps-pills" role="tablist" aria-label="Filter apps">
        {Object.entries(PILL_LABELS).map(([k, v], i) => (
          <button key={k} className={`app-pill${i === 0 ? ' active' : ''}`} data-app-filter={k} onClick={(e) => typeof (window as any).setAppsFilter === 'function' && (window as any).setAppsFilter(k, e.currentTarget)}>{v}</button>
        ))}
      </div>
